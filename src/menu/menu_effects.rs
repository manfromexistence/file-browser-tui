...
impl EffectsRepository {
    pub(super) fn new(theme: ChatTheme, rng: &mut SimpleRng) -> Self {
        ...
        let opening_effects = vec![
            ("sweep in left to right", fx::sweep_in(Motion::LeftToRight, 30, 0, screen_bg, (slow, QuadOut))),
            ("sweep in right to left", fx::sweep_in(Motion::RightToLeft, 30, 0, screen_bg, (slow, QuadOut))),
            ...
            ("slide in from left", fx::slide_in(Motion::LeftToRight, 20, 0, screen_bg, (medium, QuadOut))),
            ("slide in from right", fx::slide_in(Motion::RightToLeft, 20, 0, screen_bg, (medium, QuadOut))),
            ("expand vertical", fx::expand(ExpandDirection::Vertical, Style::new().fg(color1).bg(screen_bg), 1200)),
            ("expand horizontal", fx::expand(ExpandDirection::Horizontal, Style::new().fg(color2).bg(screen_bg), 1200)),
        ...
        let closing_effects = vec![
            ...
            ("sweep out left to right", fx::sweep_out(Motion::LeftToRight, 45, 0, color3, (slow, QuadOut))),
            ("sweep out right to left", fx::sweep_out(Motion::RightToLeft, 45, 0, color4, (slow, QuadOut))),
            ("slide out to right", fx::slide_out(Motion::LeftToRight, 80, 0, screen_bg, (medium, QuadIn))),
            ...
            ("shrink vertical", fx::expand(ExpandDirection::Vertical, Style::new().fg(color2).bg(screen_bg), 1200).reversed()),
            ("shrink horizontal", fx::expand(ExpandDirection::Horizontal, Style::new().fg(color3).bg(screen_bg), 1200).reversed()),
        ];
